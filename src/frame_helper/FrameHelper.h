...
namespace frame_helper
{
    class FrameHelper
    {
        private:
            ...
            sonar_types_v2::samples::frame::Frame frame_buffer;
            sonar_types_v2::samples::frame::Frame frame_buffer2;

            //used insight convertColor
            sonar_types_v2::samples::frame::Frame frame_buffer3;
        ...
        public:
            ...
            void convert(const sonar_types_v2::samples::frame::Frame &src,
                    sonar_types_v2::samples::frame::Frame &dst, int offset_x = 0, int offset_y = 0,
	    ...
	    // which uses R and P matrices for stereo calibration as well
            ...
            //see cv::remap 
            void undistort(const sonar_types_v2::samples::frame::Frame &src, sonar_types_v2::samples::frame::Frame &dst, 
                    UndistortAlgorithm undistort_algorithm = UNDISTORT_CUBIC);

            //copies a cv::Mat into a Frame
            //frame is initialized if it has not got the right size
            static void copyMatToFrame(const cv::Mat &src, sonar_types_v2::samples::frame::Frame &frame);
            ...
            static void resize_bayer(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst,
                    int offset_x = 0, int offset_y = 0);

            //resize a frame
            //the size is implied by the dst frame
            static void resize(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst,
            ...
            static void rotateBy180Degrees(const sonar_types_v2::samples::frame::Frame &src,
            ...
            static float calcDistanceToObject(const sonar_types_v2::samples::frame::Frame &frame,
            ...
            static cv::Point2f calcRelPosToPoint(const sonar_types_v2::samples::frame::Frame &frame,
                                             int x1,int y1,int x2,int y2, float d);
            static cv::Point2f calcRelPosToCenter(const sonar_types_v2::samples::frame::Frame &frame,int x1,int y1,float d);

            //converts src to dst 
            //the mode of dst implies the conversion 
            void convertColor(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst);

            //converts a bayer pattern image to rgb image which has 8 bit data depth for each channel
            static void convertBayerToRGB24(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst);
            static void convertBayerToRGB24(const uint8_t *src, uint8_t *dst, int width, int height ,sonar_types_v2::samples::frame::frame_mode_t mode);

            //converts a rgb image to gray scale image
            static void convertRGBToGray(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst,bool copy_attributes =true);

            //copies only the green channel of a bayer pattern image to dst
            //dst has to be MODE_GRAYSCALE
            static void convertBayerToGreenChannel(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst);
            static void convertBayerToGreenChannel(const uint8_t *src, uint8_t *dst, int width, int height, sonar_types_v2::samples::frame::frame_mode_t mode);

            //calculates the diff between src1 and src2 
            //dst[i] = |src1[i]-src2[i]|
            //only 8 Bit data depth is supported 
            static void calcDiff(const sonar_types_v2::samples::frame::Frame &src1,const sonar_types_v2::samples::frame::Frame &src2,sonar_types_v2::samples::frame::Frame &dst);
			
	    /**
	     * /Brief conversion for PJPG to RGB24.
	    */
	    static void convertPJPGToRGB24(const sonar_types_v2::samples::frame::Frame &src, sonar_types_v2::samples::frame::Frame &dst);
	    ...
	    static cv::Mat convertToCvMat(const sonar_types_v2::samples::frame::Frame &frame);
	    
	    static int getOpenCvType(const sonar_types_v2::samples::frame::Frame &frame);
    
            void saveFrame(const std::string &filename,sonar_types_v2::samples::frame::Frame const &frame);
            void loadFrame(const std::string &filename,sonar_types_v2::samples::frame::Frame &frame);

            /**
             * Loads a JPEG image from file without decompressing it (can be useful if frame is directly transmitted afterwards)
             */
            static void loadFrameJPEG(const std::string &filename, sonar_types_v2::samples::frame::Frame &frame);
            /**
             ...
             */
            static void loadFrameJPEG(const uint8_t* src, size_t size, sonar_types_v2::samples::frame::Frame &frame);
    };
};
