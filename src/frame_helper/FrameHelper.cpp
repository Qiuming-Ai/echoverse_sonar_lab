...
using namespace sonar_types_v2::samples::frame;
...
namespace frame_helper
{
    ...
    int FrameHelper::getOpenCvType(const sonar_types_v2::samples::frame::Frame& frame)
    {
	int itype = 0;
    ...
    cv::Mat FrameHelper::convertToCvMat(const sonar_types_v2::samples::frame::Frame &frame)
    {
    ...
    void FrameHelper::convert(const sonar_types_v2::samples::frame::Frame &src,
            sonar_types_v2::samples::frame::Frame &dst,
            int offset_x,
            int offset_y,
    ...
    void FrameHelper::undistort(const sonar_types_v2::samples::frame::Frame &src,
            sonar_types_v2::samples::frame::Frame &dst,
            UndistortAlgorithm undistort_algorithm)
    {
    ...
    void FrameHelper::resize(const sonar_types_v2::samples::frame::Frame &src,
            sonar_types_v2::samples::frame::Frame &dst,
 ...
 yet, set 'width' and 'height' in the startup-script accordingly to the delivered camera image");
    ...
    void FrameHelper::resize_bayer(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst,
            int offset_x,int offset_y)
    {
    ...
    void FrameHelper::convertColor(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst)
    {
        ...
        case MODE_PJPG:
        {
            ...
            case MODE_JPEG:
            {
                sonar_types_v2::samples::frame::Frame frame_tmp;
        ...
        case MODE_BAYER_GBRG:
            ...
            case MODE_JPEG:
            {
                sonar_types_v2::samples::frame::Frame frame_tmp(src.getWidth(), src.getHeight(), src.getDataDepth(), sonar_types_v2::samples::frame::MODE_RGB, -1);
        ...
        case MODE_JPEG:
        {
            ...
            case MODE_RGB:
            {
                jpeg_conversion.decompress(src, sonar_types_v2::samples::frame::MODE_RGB, dst);
                break;
            }

            //JPEG --> BGR
            case MODE_BGR:
                jpeg_conversion.decompress(src, sonar_types_v2::samples::frame::MODE_BGR, dst);
                break;
            ...
            //JPEG --> grayscale
            case MODE_GRAYSCALE:
                jpeg_conversion.decompress(src, sonar_types_v2::samples::frame::MODE_GRAYSCALE, dst);
                break;
    ...
    void FrameHelper::convertRGBToGray(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst,bool copy_attributes)
    {
    ...
    cv::Point2f FrameHelper::calcRelPosToPoint(const sonar_types_v2::samples::frame::Frame &frame,
    ...
    cv::Point2f FrameHelper::calcRelPosToCenter(const sonar_types_v2::samples::frame::Frame &frame,
            int x1,int y1, float d)
    {
        return calcRelPosToPoint(frame,x1,y1,frame.getWidth()*0.5,frame.getHeight()*0.5,d);
    }

    float FrameHelper::calcDistanceToObject(const sonar_types_v2::samples::frame::Frame &frame,
    ...
    void FrameHelper::convertBayerToRGB24(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst)
    {
        dst.init(src.getWidth(),src.getHeight(),src.getDataDepth(),MODE_RGB, -1);
        convertBayerToRGB24(src.getImageConstPtr(),dst.getImagePtr(),src.getWidth(),src.getHeight(),src.frame_mode);
        dst.copyImageIndependantAttributes(src);
    ...
    void FrameHelper::convertBayerToRGB24(const uint8_t *src, uint8_t *dst, int width, int height, sonar_types_v2::samples::frame::frame_mode_t mode)
    {
    ...
    void FrameHelper::calcDiff(const sonar_types_v2::samples::frame::Frame &src1,const sonar_types_v2::samples::frame::Frame &src2,sonar_types_v2::samples::frame::Frame &dst)
    {
    ...
    void FrameHelper::convertBayerToGreenChannel(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst)
    {
        //check dst format
    ...
    void FrameHelper::convertBayerToGreenChannel(const uint8_t *src, uint8_t *dst, int width, int height, sonar_types_v2::samples::frame::frame_mode_t mode)
    {
        const int srcStep = width;
    ...
    void FrameHelper::convertPJPGToRGB24(const sonar_types_v2::samples::frame::Frame &src,sonar_types_v2::samples::frame::Frame &dst)
    {
        //check src format
    ...
    void FrameHelper::copyMatToFrame(const cv::Mat &src, sonar_types_v2::samples::frame::Frame &frame)
    {
        frame_mode_t mode;
        int color_depth;
        switch (src.type())
            ...
            size_t const src_size, int const width, int const height)
    {
        ...
        jpeg_conversion.decompress(src, src_size, width, height,
                sonar_types_v2::samples::frame::MODE_RGB, dst);
    }

    void FrameHelper::rotateBy180Degrees(const sonar_types_v2::samples::frame::Frame &src,
            sonar_types_v2::samples::frame::Frame &dst)
    {
        dst.init(src, false);
    ...
    void FrameHelper::saveFrame(const std::string &filename,const sonar_types_v2::samples::frame::Frame &frame)
    {
        sonar_types_v2::samples::frame::Frame temp;
    ...
    void FrameHelper::loadFrame(const std::string &filename,sonar_types_v2::samples::frame::Frame &frame)
    {
        cv::Mat mat = cv::imread(filename.c_str());
        copyMatToFrame(mat,frame);
    }

    void FrameHelper::loadFrameJPEG(const std::string &filename, sonar_types_v2::samples::frame::Frame &frame)
    {
        conversion::JpegConversion::loadJpeg(filename, 0, 0, frame);
    }
    void FrameHelper::loadFrameJPEG(const uint8_t* src, size_t size, sonar_types_v2::samples::frame::Frame &frame)
    {
