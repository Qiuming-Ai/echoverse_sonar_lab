...
This online core produces real-time beam/bin intensity samples and supports the
range/intensity representation used for echo point-cloud recovery. It does **not**
itself synthesize the final multi-channel receive waveform. The separately linked
native library under `src/offline_processing/` consumes ESL3D point-cloud data and
performs waveform synthesis, Doppler/noise processing, HDF5 export, matched filtering,
TVG, and beamforming.
...
"**geometry/material scene -> real-time acoustic intensity grid**".
...
The word "echo" in the GUI refers to the rendered echo/intensity image or its
point-cloud representation. It must not be interpreted as the final raw channel echo
waveform, which is generated later by the native offline pipeline.
