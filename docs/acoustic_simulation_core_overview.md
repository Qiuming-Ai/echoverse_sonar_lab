...
This C++ core produces real-time beam/bin intensity samples and supports the
range/intensity representation used for echo point-cloud recovery. It does **not**
synthesize the final multi-channel receive waveform used by the offline MATLAB path.
MATLAB consumes C++-generated ESL3D point-cloud data and performs waveform synthesis,
Doppler/noise processing, HDF5 export, matched filtering, TVG, and beamforming.
...
The word "echo" in the C++ GUI refers to the rendered echo/intensity image or its
point-cloud representation. It must not be interpreted as the final raw channel echo
waveform, which is generated by the MATLAB offline pipeline.
